...
int main()
{
	...
	last falg[1000];
	HANDLE h[2];	
	(HANDLE)_beginthreadex(0, 0, (unsigned int(__stdcall*)(void*))cust, &zc, 0, 0);
	//	cust();
	(HANDLE)_beginthreadex(0, 0, (unsigned int(__stdcall*)(void*))orde, &zo, 0, 0);
	//	orde();
	(HANDLE)_beginthreadex(0, 0, (unsigned int(__stdcall*)(void*))line, &zl, 0, 0);
	//	line();
	while (1)
	{
		if (zc == 52&&zo==52&&zl==52)break;
	}
	printf("loading sucessed..\n");
	connct();
	scanf("%d", &flag);
	for (i = 0; i < flag; i++)
	{
